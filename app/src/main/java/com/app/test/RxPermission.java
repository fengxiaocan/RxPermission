...
public class RxPermission {
    ...
    private LinkedList<PermissionOption> permissionList;
    ...
    private RxPermission addOption(PermissionOption option) {
    ...
    private void showDialog(final PermissionOption option) {
        CommonAlertDialog.builder(activity)
                         .setTitle(title)
                         .setMessage(option.reminder())
                         .setCancelable(cancelable)
                         .setCanceledOnTouchOutside(cancelable)
                         .setMiddleButton(button, new DialogInterface.OnClickListener() {
                             @Override
                             public void onClick(DialogInterface dialog, int which) {
                                 dialog.dismiss();
                                 PermissionOption data = option;
                                 if (data instanceof RxRunTimeOption) {
                                     runtime((RxRunTimeOption) data, false);
                                 } else if (data instanceof RxNotificationOption) {
                                     notification((RxNotificationOption) data, false);
                                 } else if (data instanceof RxSettingOption) {
                                     setting((RxSettingOption) data, false);
                                 } else if (data instanceof RxInstallOption) {
                                     install((RxInstallOption) data, false);
                                 } else if (data instanceof RxOverlayOption) {
                                     overlay((RxOverlayOption) data, false);
                                 }
                             }
                         })
                         .show();
    ...
    private static class RuntimeAction extends MyAction<List<String>> {
        ...
        public void onAction(final List<String> data) {
            ...
            if (option1.goSetting) {
                if (TextUtils.isEmpty(option1.goSettingRemind)) {
                    ...
                    CommonAlertDialog.builder(option.rxPermission.activity)
                                     .setTitle(option.rxPermission.title)
                                     .setMessage(option.reminder())
                                     .setCancelable(false)
                                     .setCanceledOnTouchOutside(false)
                                     .setMiddleButton("前往系统设置",
                                             new CommonAlertDialog.CommonDialogListener() {
                                                 @Override
                                                         ...
                                                         int which)
                                                 {
                                                     ...
                                                     AndPermission.with(
                                                             option.rxPermission.activity)
                                                                  .runtime()
                                                                  .setting()
                                                                  .start(((RxRunTimeOption) option).requestCode);
                                                     RuntimeAction.super.onAction(data);
                                                 }
                                             })
                                     .show();
                }
    ...
    static class MyAction<T> implements Action<T> {
    ...
    static class SampleRationale<T> implements Rationale<T> {
        @Override
    ...
    public static abstract class PermissionOption<T> {
    ...
    public static class RxNotificationOption extends PermissionOption<RxNotificationOption> {
    ...
    public static class RxOverlayOption extends PermissionOption<RxOverlayOption> {
    ...
    public static class RxSettingOption extends PermissionOption<RxSettingOption> {
    ...
    public static class RxInstallOption extends PermissionOption<RxInstallOption> {
        private File file;

        private RxInstallOption(RxPermission rxPermission, File file) {
            super(rxPermission);
            this.file = file;
        }
    ...
    public static class RxRunTimeOption extends PermissionOption<RxRunTimeOption> {
        int requestCode;
        boolean goSetting;
        String goSettingRemind;
        private String[] permissionGroup;

        private RxRunTimeOption(RxPermission rxPermission, String... permission) {
            super(rxPermission);
